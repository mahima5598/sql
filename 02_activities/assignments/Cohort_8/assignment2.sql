...
SELECT
*,
dense_rank() OVER(PARTITION by customer_id ORDER BY market_date ) as customer_market_visit
FROM (
...
Worst_Day AS (
	...
	SELECT
		market_date,
		total_sales,
		RANK() OVER(ORDER BY total_sales ASC) as num_rank
	FROM SalesPerDate
...
Best_Day AS (
	...
	SELECT
		market_date,
		total_sales,
		RANK() OVER(ORDER BY total_sales DESC) as num_rank
	FROM SalesPerDate
...
SELECT
	market_date,
	total_sales,
	'Highest total sales' AS sale_status
FROM Best_Day
WHERE num_rank = 1
...
SELECT
	market_date,
	total_sales,
	'Lowest total sales' AS sale_status
FROM Worst_Day
WHERE num_rank = 1;
...
SELECT
    ...
    product_name,
    original_price,
    (5 * original_price) * COUNT(DISTINCT c.customer_id) AS total_money_per_product
FROM
    vendor_inventory vi
CROSS JOIN
    customer c
...
WHERE product_id = 24 AND product_name = 'Whole Wheat Bread';
